...
def test_generate_code_stream_emits_reasoning_and_final_response():
    ...
    with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}, clear=False):
        with patch("backend.copilot.orchestrator.window_conversation_history", return_value=[]):
            with patch("backend.copilot.orchestrator._get_allowed_function_names", return_value={"s"}):
                with patch("backend.copilot.orchestrator.should_prefetch_kb", return_value=False):
                    with patch("backend.copilot.orchestrator.detect_sound_types", return_value=([], False)):
                        with patch("backend.copilot.orchestrator.should_enable_web_search", return_value=False):
                            with patch("backend.copilot.orchestrator.get_all_preset_names", return_value={"bd", "sd"}):
                                with patch("backend.copilot.orchestrator.get_function_signatures", return_value={}):
                                    with patch("backend.copilot.orchestrator._log_interaction"):
                                        with patch(
                                            "backend.copilot.orchestrator.generate_with_context_stream",
                                            _make_streaming_llm(
                                                's("bd sd")',
                                                "Built a drum loop.",
                                                events=[
                ...
                with patch("backend.copilot.orchestrator.should_prefetch_kb", return_value=False):
                    with patch("backend.copilot.orchestrator.detect_sound_types", return_value=([], False)):
                        with patch("backend.copilot.orchestrator.should_enable_web_search", return_value=False):
                            with patch("backend.copilot.orchestrator.get_all_preset_names", return_value={"bd", "sd"}):
                                with patch("backend.copilot.orchestrator.get_function_signatures", return_value={}):
                                    with patch("backend.copilot.orchestrator.retrieve_context_for_functions", return_value=""):
                                        with patch("backend.copilot.orchestrator.retrieve_preset_context", return_value=""):
                                            with patch("backend.copilot.orchestrator._log_interaction"):
                                                with patch(
                                                    "backend.copilot.orchestrator.generate_with_context_stream",
                                                    _make_streaming_llm(
                                                    ...
                                                    with patch(
                                                        "backend.copilot.orchestrator.repair_with_context_stream",
                                                        _make_streaming_llm(
                                                            's("bd")',
